...
def run_torchsim_nve(
    ...
    """Load TorchSim model, run NVE MD for MD_STEPS per n/iter, return timings."""
...
def run_ase_nve(
    ...
    """Load ASE calculator, run NVE MD for MD_STEPS per n/iter, return timings."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ase_calc = mace_mp(
        model=MaceUrls.mace_mpa_medium,
        default_dtype="float64",
    ...
    for it in range(iterations):
        ...
        for n in n_structures_list:
            ...
            elapsed = time.perf_counter() - t0
            times.append(elapsed)
            print(f"  n={n} ase_nve_time={elapsed:.6f}s")
    return times
...
def plot_results(
    ...
    output_path: str = "speedup_nve.html",
) -> None:
    """Plot TorchSim and ASE NVE MD timings per iteration and save to HTML."""
    ...
    fig.update_layout(
        title=f"NVE MD timings ({MD_STEPS} steps)",
...
if __name__ == "__main__":
    ...
    ts_times = run_torchsim_nve(n_structures_list, iterations, base_structure)
    ase_times = run_ase_nve(n_structures_list, iterations, mgo_ase)
