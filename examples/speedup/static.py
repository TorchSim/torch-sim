...
def run_torchsim_static(
    ...
    """Load TorchSim model, run static for each n and iteration, return timings."""
...
def run_ase_static(
    ...
    """Load ASE calculator, run static for each n and iteration, return timings."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ase_calc = mace_mp(
        model=MaceUrls.mace_mpa_medium,
        default_dtype="float64",
    ...
    for it in range(iterations):
        ...
        for n in n_structures_list:
            ...
            elapsed = time.perf_counter() - t0
            times.append(elapsed)
            print(f"  n={n} ase_static_time={elapsed:.6f}s")
    return times
...
def plot_results(
    ...
    """Plot TorchSim and ASE timings per iteration and save to HTML."""
    n = len(n_structures_list)
    fig = go.Figure()
    for it in range(iterations):
        start, end = it * n, (it + 1) * n
        ts_times = static_times[start:end]
