...
* Graph indices (for processing multiple systems simultaneously)
...
print(f"State has {si_state.n_atoms} atoms and {si_state.n_graphs} graphs")
...
print(f"Graph indices shape: {si_state.graph_idx.shape}")
...
* Batchwise attributes are tensors with shape (n_graphs, ...), this is just `cell` for
...
print(
    f"Multi-state has {multi_state.n_atoms} total atoms across {multi_state.n_graphs} graphs"
)
...
print(f"Graph indices shape: {multi_state.graph_idx.shape}")
...
print(f"This state has {multi_state_copy.n_graphs} graphs")
...
print(
    ...
    f"{multi_state_copy.n_graphs} graphs in the original state"
)

# we can put them back together with concatenate
multi_state_full = ts.concatenate_states([*popped_states, multi_state_copy])
print(f"Again we have {multi_state_full.n_graphs} graphs in the full state")
...
md_state = MDState(
    **asdict(si_state),  # Copy all SimState properties
    ...
    energy=torch.zeros((si_state.n_graphs,), device=si_state.device),  # Initial 0 energy
)
