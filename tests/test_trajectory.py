...
def random_state() -> MDState:
    ...
    return MDState(
        ...
        graph_idx=torch.zeros(10, dtype=torch.int32),
        pbc=True,
    )
...
def test_property_model_consistency(
    ...
    for graph_idx in range(2):
        ...
        single_state = si_double_sim_state[graph_idx]
        reporter = TrajectoryReporter(
            tmp_path / f"single_{graph_idx}.hdf5",
            state_frequency=1,
            prop_calculators=prop_calculators,
        )
        # Run one step
        ...
        single_reporters.append(
            TorchSimTrajectory(tmp_path / f"single_{graph_idx}.hdf5", mode="r")
        )
    ...
    # Compare property values between single and multi-graph approaches
    for graph_idx in range(2):
        single_ke = single_reporters[graph_idx].get_array("ones")[0]
        multi_ke = multi_trajectories[graph_idx].get_array("ones")[0]
        assert torch.allclose(torch.tensor(single_ke), torch.tensor(multi_ke))

        single_com = single_reporters[graph_idx].get_array("center_of_mass")[0]
        multi_com = multi_trajectories[graph_idx].get_array("center_of_mass")[0]
...
def test_reporter_with_model(
    ...
    assert len(props) == 2  # One dict per graph
    for graph_props in props:
        assert set(graph_props) == {"energy"}
        assert isinstance(graph_props["energy"], torch.Tensor)
        assert graph_props["energy"].shape == (1,)
        assert graph_props["energy"] == pytest.approx(49.4150)
    ...
    for graph_idx, trajectory in enumerate(trajectories):
        # Get the property value from file
        file_energy = trajectory.get_array("energy")[0]
        graph_props = props[graph_idx]

        # Calculate expected value
        substate = si_double_sim_state[graph_idx]
        ...
        np.testing.assert_allclose(graph_props["energy"], expected)
        # Compare returned properties with file contents
        np.testing.assert_allclose(graph_props["energy"], file_energy)
