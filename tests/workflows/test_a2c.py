...
def test_get_diameter_matrix(device):
    # Test with a single element (Cu)
    comp = Composition("Cu")
...
def test_get_subcells_to_crystallize(device):
    # Create test data
...
def test_subcells_to_structures(device):
    # Create test data
    frac_positions = torch.tensor(
        [
            [0.1, 0.1, 0.1],
    ...
    cell = torch.tensor([[5.0, 0.0, 0.0], [0.0, 5.0, 0.0], [0.0, 0.0, 5.0]], device=device)
    species = ["Cu", "Cu", "O", "O", "O"]
