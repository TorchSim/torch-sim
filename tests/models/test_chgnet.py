...
def test_chgnet_batched_calculations() -> None:
    ...
    assert result["forces"].shape == (
        si_atoms.get_global_number_of_atoms() + cu_atoms.get_global_number_of_atoms(),
        3,
    )
...
def test_chgnet_single_vs_batched_consistency() -> None:
    ...
    assert torch.allclose(
        single_result["energy"][0], batched_result["energy"][0], atol=1e-5
    )
    assert torch.allclose(
        single_result["energy"][0], batched_result["energy"][1], atol=1e-5
    )
    assert torch.allclose(
        single_result["forces"],
        batched_result["forces"][: single_state.n_atoms],
        atol=1e-5,
    )
    assert torch.allclose(
        single_result["forces"],
    ...
    assert torch.allclose(
        single_result["stress"][0], batched_result["stress"][0], atol=1e-5
    )
    assert torch.allclose(
        single_result["stress"][0], batched_result["stress"][1], atol=1e-5
    )
