...
def test_concatenate_states_no_side_effects(si_sim_state: SimState) -> None:
    ...
    assert torch.allclose(state1.positions, original_positions1), (
        "First input state was modified!"
    )
    assert torch.allclose(state2.positions, original_positions2), (
        "Second input state was modified!"
    )
    ...
    assert (
        concatenated.positions.shape[0]
        == state1.positions.shape[0] + state2.positions.shape[0]
    ), "Concatenated state has wrong number of atoms!"
...
def test_initialize_state_no_side_effects(si_sim_state: SimState) -> None:
    ...
    # Verify original state is unchanged,
    # i.e. the original state is not modified by the initialize_state function
    assert torch.allclose(si_sim_state.positions, original_positions), (
        "Original state was modified!"
    )
    assert si_sim_state.dtype == original_dtype, "Original state dtype was changed!"
