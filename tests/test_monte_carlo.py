...
def test_generate_permutation(
    ...
    validate_permutation(permutation, batched_diverse_state.graph_idx)
...
def test_generate_swaps(batched_diverse_state: ts.SimState, generator: torch.Generator):
    ...
    # Check swaps are within same graph
    graph_idx = batched_diverse_state.graph_idx
    assert torch.all(graph_idx[swaps[:, 0]] == graph_idx[swaps[:, 1]])
...
def test_validate_permutation(batched_diverse_state: ts.SimState):
    ...
    permutation = swaps_to_permutation(swaps, batched_diverse_state.n_atoms)
    validate_permutation(permutation, batched_diverse_state.graph_idx)  # Should not raise
    ...
    if batched_diverse_state.n_atoms > 2:
        ...
        with pytest.raises(ValueError, match="Swaps must be between"):
            validate_permutation(invalid_perm, batched_diverse_state.graph_idx)
...
def test_monte_carlo(
    ...
    # Verify graph_idx assignments remain unchanged
    assert torch.all(current_state.graph_idx == batched_diverse_state.graph_idx)

    # Verify atomic numbers distribution remains the same per graph
    for idx in torch.unique(current_state.graph_idx):
        graph_mask_orig = batched_diverse_state.graph_idx == idx
        graph_mask_result = current_state.graph_idx == idx

        orig_counts = torch.bincount(
            batched_diverse_state.atomic_numbers[graph_mask_orig]
        )
        result_counts = torch.bincount(current_state.atomic_numbers[graph_mask_result])
