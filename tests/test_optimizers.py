...
def test_fire_optimization(
    ...
    current_sim_state = ts.SimState(
        ...
        graph_idx=ar_supercell_sim_state.graph_idx.clone(),
    )
...
def test_fire_ase_negative_power_branch(
    ...
    initial_dt_batch = state.dt.clone()  # per-graph dt
    ...
    expected_final_velocities = (
        expected_dt_val * forces_at_power_calc[updated_state.graph_idx == 0]
    )
    assert torch.allclose(
        updated_state.velocities[updated_state.graph_idx == 0],
        expected_final_velocities,
        atol=1e-6,
    )
...
def test_fire_vv_negative_power_branch(
    ...
    assert torch.allclose(
        updated_state.velocities[updated_state.graph_idx == 0],
        torch.zeros_like(updated_state.velocities[updated_state.graph_idx == 0]),
        atol=1e-7,
    )
...
def test_cell_optimizer_init_cell_factor_none(
    ...
    # Ensure n_graphs > 0 for cell_factor calculation from counts
    assert ar_supercell_sim_state.n_graphs > 0
    opt_state = init_fn(ar_supercell_sim_state)  # Uses ts.SimState directly
    assert isinstance(opt_state, expected_state_type)
    _, counts = torch.unique(ar_supercell_sim_state.graph_idx, return_counts=True)
...
def test_fire_fixed_cell_unit_cell_consistency(  # noqa: C901
    ...
    for step, energy_unit_cell in enumerate(individual_energies_unit_cell):
        assert abs(energy_unit_cell - individual_energies_fire[step]) < 1e-4, (
            f"Energy for graph {step} doesn't match position only optimization: "
            f"graph={energy_unit_cell}, individual={individual_energies_fire[step]}"
        )
