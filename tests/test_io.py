...
def test_multiple_structures_to_state(
    ...
    assert state.graph_idx.shape == (16,)
    assert torch.all(
        state.graph_idx == torch.repeat_interleave(torch.tensor([0, 1], device=device), 8)
    )
...
def test_single_atoms_to_state(si_atoms: Atoms, device: torch.device) -> None:
    ...
    assert state.graph_idx.shape == (8,)
    assert torch.all(state.graph_idx == 0)
...
def test_state_round_trip(
    ...
    unique_graphs = torch.unique(sim_state.graph_idx)

    # Convert to intermediate format
    intermediate_format = to_format_fn(sim_state)
    assert len(intermediate_format) == len(unique_graphs)
    ...
    assert torch.all(sim_state.graph_idx == round_trip_state.graph_idx)
