...
def measure_model_memory_forward(state: SimState, model: ModelInterface) -> float:
    ...
    print(  # noqa: T201
        ...
        f"{state.n_atoms} atoms and {state.n_graphs} graphs.",
    )
    # Clear GPU memory
...
def determine_max_batch_size(
    ...
    for i in range(len(sizes)):
        n_graphs = sizes[i]
        concat_state = concatenate_states([state] * n_graphs)
...
def estimate_max_memory_scaler(
    ...
    **kwargs: Any,
    ...
    print(  # noqa: T201
        ...
        f"and {max_state.n_graphs} batches, and smallest system has "
        f"{min_state.n_atoms} atoms and {min_state.n_graphs} batches.",
    )
...
class BinningAutoBatcher:
    ...
    Attributes:
        ...
        max_memory_scaler (float): Maximum memory metric allowed per graph.
        ...
        *,
        ...
        Args:
            ...
            memory_scales_with ("n_atoms" | "n_atoms_x_density"): Metric to use
                ...
                Defaults to "n_atoms_x_density".
            max_memory_scaler (float | None): Maximum metric value allowed per graph. If
...
class InFlightAutoBatcher:
        ...
        *,
        ...
        Args:
            ...
            memory_scales_with ("n_atoms" | "n_atoms_x_density"): Metric to use
                ...
                - "n_atoms_x_density": Uses atom count multiplied by number density
                Defaults to "n_atoms_x_density".
            max_memory_scaler (float | None): Maximum metric value allowed per graph.
    ...
    def _get_first_batch(self) -> SimState:
        ...
        if not has_max_metric:
            n_graphs = determine_max_batch_size(
                first_state,
                self.model,
                max_atoms=self.max_atoms_to_try,
                scale_factor=self.memory_scaling_factor,
            )
            self.max_memory_scaler = n_graphs * first_metric * 0.8
    ...
    def next_batch(  # noqa: C901
        ...
        Args:
            ...
            convergence_tensor (torch.Tensor | None): Boolean tensor with shape
                [n_graphs] indicating which states have converged (True) or not
        ...
        if len(convergence_tensor) != updated_state.n_graphs:
            raise ValueError(f"{len(convergence_tensor)=} != {updated_state.n_graphs=}")
        ...
        if updated_state.n_graphs <= 0:
            raise ValueError(f"{updated_state.n_graphs=} <= 0")
