...
def nvt_langevin(  # noqa: C901
    ...
    *,
    ...
    Args:
        ...
        dt (torch.Tensor): Integration timestep, either scalar or with shape [n_graphs]
        ...
        gamma (torch.Tensor, optional): Friction coefficient for Langevin thermostat,
            either scalar or with shape [n_graphs]. Defaults to 1/(100*dt).
    ...
    def ou_step(
        ...
        Args:
            ...
            gamma (torch.Tensor): Friction coefficient controlling noise strength,
                either scalar or with shape [n_graphs]
        ...
        if isinstance(kT, torch.Tensor) and len(kT.shape) > 0:
            # kT is a tensor with shape (n_graphs,)
            kT = kT[state.graph_idx]

        # Index c1 and c2 with state.graph_idx to align shapes with state.momenta
        if isinstance(c1, torch.Tensor) and len(c1.shape) > 0:
            c1 = c1[state.graph_idx]
    ...
    *,
    ...
    def nvt_nose_hoover_init(
        ...
        **kwargs: Any,
        ...
        # Calculate initial kinetic energy per graph
        KE = calc_kinetic_energy(momenta, state.masses, graph_idx=state.graph_idx)
        ...
        # For now, sum the per-graph DOF as chain expects a single int
    ...
    def nvt_nose_hoover_update(
        ...
        # Update chain kinetic energy per graph
...
def nvt_nose_hoover_invariant(
    ...
    """
    # Calculate system energy terms per graph
    e_pot = state.energy
    e_kin = calc_kinetic_energy(state.momenta, state.masses, graph_idx=state.graph_idx)

    # Get system degrees of freedom per graph
    n_atoms_per_graph = torch.bincount(state.graph_idx)
