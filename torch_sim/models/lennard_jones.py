...
class LennardJonesModel(torch.nn.Module, ModelInterface):
    ...
    def forward(self, state: ts.SimState | StateDict) -> dict[str, torch.Tensor]:
        ...
        dispatching each graph to the unbatched implementation and combining results.
        ...
        Raises:
            ValueError: If graph cannot be inferred for multi-cell systems.
        ...
        Example::
            ...
            energy = results["energy"]  # Shape: [n_graphs]
            forces = results["forces"]  # Shape: [n_atoms, 3]
            stress = results["stress"]  # Shape: [n_graphs, 3, 3]
        ...
        if state.graph_idx is None and state.cell.shape[0] > 1:
            raise ValueError("Graph can only be inferred for batch size 1.")
