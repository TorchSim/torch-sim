...
class FairChemModel(torch.nn.Module, ModelInterface):
    ...
    def forward(self, state: ts.SimState | StateDict) -> dict:
        ...
        if state.graph_idx is None:
            state.graph_idx = torch.zeros(state.positions.shape[0], dtype=torch.int)
        ...
        natoms = torch.bincount(state.graph_idx)
        fixed = torch.zeros((state.graph_idx.size(0), natoms.sum()), dtype=torch.int)
        data_list = []
