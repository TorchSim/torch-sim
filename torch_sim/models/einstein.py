...
class EinsteinModel(ModelInterface):
    ...
    def get_spring_constants(self) -> torch.Tensor:
        """Get the spring constants for each atom in the Einstein model.

        Returns:
            Tensor of shape [N] with spring constants k_i = m_i * omega_i^2
            for each atom.
        """
        return self.masses * (self.frequencies**2)  # [N]
